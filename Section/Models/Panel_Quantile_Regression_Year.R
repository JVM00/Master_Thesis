...
save(QR_Y_npl_hh.nfi,file=paste(path_data,'QR_Y_npl_hh.nfi.RData', sep = ""))
save(QR_Y_npl_hh,file=paste(path_data,'QR_Y_npl_hh.RData', sep = ""))
save(QR_Y_npl_nfi,file=paste(path_data,'QR_Y_npl_nfi.RData', sep = ""))
save(QR_Y_npl_fi,file=paste(path_data,'QR_Y_npl_fi.RData', sep = ""))
...
for (i in names(anova)){
 ...
 print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_Y_",d,".html", sep = ""))
print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_Y_",d,".tex", sep = ""))
...
QR_npl_hh <- QR_Y_npl_hh %>%filter(!grepl('Year', var))%>%  mutate(Exposure = "Households")
QR_npl_nfi <- QR_Y_npl_nfi %>%filter(!grepl('Year', var))%>% mutate(Exposure = "Non-Financial Corp.")
QR_npl_fi <- QR_Y_npl_fi %>%filter(!grepl('Year', var))%>% mutate(Exposure = "Financial Corp.")
#View(QR_Y_npl_hh)
#View(QR_Y_npl_hh)
...
for (i in names(sets)){
 ...
 # class( pfe2.table2$pfe.types)
  ...
  assign(paste("QRFE1_Y_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh)
 ...
 # x <- xtable(pfe3.table2, caption = paste("A Fixed Effects Quantile Regression (2014-2019)[$\lambda = 1$]:",k,sep=" "))
  ...
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_Y_",d,lambda,".html", sep = ""))
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_y_",d,lambda,".tex", sep = ""))
...
save(QRFE1_Y_npl_fi,file=paste(path_data,'QRFE1_Y_npl_fi.RData', sep = ""))

save(QRFE1_Y_npl_hh,file=paste(path_data,'QRFE1_Y_npl_hh.RData', sep = ""))
save(QRFE1_Y_npl_nfi,file=paste(path_data,'QRFE1_Y_npl_nfi.RData', sep = ""))
...
QRFE1_hh <- QRFE1_Y_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")
...
path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QRFE1_npl_hh.nfi,file=paste(path_data,'QRFE1_npl_hh.nfi.RData', sep = ""))
save(QRFE5_Y_npl_fi,file=paste(path_data,'QRFE5_Y_npl_fi.RData', sep = ""))

save(QRFE5_Y_npl_hh,file=paste(path_data,'QRFE5_Y_npl_hh.RData', sep = ""))
save(QRFE5_Y_npl_nfi,file=paste(path_data,'QRFE5_Y_npl_nfi.RData', sep = ""))

QRFE5_hh <- QRFE5_Y_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")
...
for (i in names(sets)){
  ...
  assign(paste("QRFE2.5_Y_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh)
...
QRFE2.5_hh <- QRFE2.5_Y_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")
