...
#Households
ols.1 <-  OLS_75_plm$npl_hh.75
re.1 <-  RE_75_plm$npl_hh.75
fe.1 <-  FE_75_plm$npl_hh.75
sysgmm.1 <-  sysgmm4_75_plm$npl_hh.75

#Non-Financial Institutions
ols.2 <-  OLS_75_plm$npl_nfi.75
re.2 <-  RE_75_plm$npl_nfi.75
fe.2 <-  FE_75_plm$npl_nfi.75
#diffgmm.2 <- diffgmm2_plm$npl_nfi
diffgmm.2 <- sysgmm4_plm$npl_nfi.75

#Financial Institutions
ols.3 <-  OLS_75_plm$npl_fi.75
re.3 <-  RE_75_plm$npl_fi.75
fe.3 <-  FE_75_plm$npl_fi.75

#diffgmm.3 <- diffgmm2_plm$npl_fi
diffgmm.3 <- sysgmm4_plm$npl_fi.75
...
se1 = list(olsHC.1b[,"Std. Error"],reHC.1b[,"Std. Error"],feHC.1b[,"Std. Error"],olsHC.2b[,"Std. Error"],reHC.2b[,"Std. Error"], feHC.2b[,"Std. Error"],olsHC.3b[,"Std. Error"],reHC.3b[,"Std. Error"],feHC.3b[,"Std. Error"]) 

options(scipen=0, digits=3)
haus_test.hh <- phtest(FE_75_plm[[1]], RE_75_plm[[1]])
haus_test.fi <-phtest(FE_75_plm[[2]], RE_75_plm[[2]])
haus_test.nfi <-phtest(FE_75_plm[[3]], RE_75_plm[[3]])
...
####Breusch-Pagan Lagrange Multiplier Test: Test on Individual Effects###

BP.test.hh <- plmtest (ols.1)
BP.test.nfi <-plmtest (ols.2)
BP.test.fi <-plmtest (ols.3)

bp <- cbind(round(BP.test.hh$statistic,2),"","", round(BP.test.nfi$statistic,2),"","",round(BP.test.fi$statistic,2),"","")
bp.star <- cbind(round(BP.test.hh$p.value,4),"","", round(BP.test.nfi$p.value,4),"","",round(BP.test.fi$p.value,4),"","")



bp <- as.vector(c("Breusch-Pagan Test",bp,"","",""))
bp.star <- as.vector(c("Prob (Breusch-Pagan Test)",bp.star,"","",""))
...
# diffgmm.3 <- diffgmm2_plm$npl_fi
  ...
  stargazer(ols.1,re.1,  fe.1,ols.2,re.2 ,fe.2 ,ols.3, re.3,fe.3,sysgmm.1, diffgmm.2,diffgmm.3,title="Panel Data Models  (2014-2019)",
             align=TRUE,type = "latex",
             se = se1,
            column.labels=c("Households","Non-Financial","Financial","Households","Non-Financial","Financial"),
             column.separate = c(3,3,3,1,1,1),
             font.size = "small",
            ...
            model.numbers          = FALSE,
            add.lines = list(bp,bp.star,ht,ht.star,st,st.star,ar1,ar1.star,ar2,ar2.star),
            #add.lines = list(st1,ar1,ar2),
             out="Research_Proposal/Progress/Tables/Final/Panel/Static_Models_SELEC_ext_75.tex")
  
  capture.output( stargazer(ols.1,re.1,  fe.1,ols.2,re.2 ,fe.2 , ols.3,re.3,fe.3,sysgmm.1, diffgmm.2,diffgmm.3, title="Panel Data Models (2014-2019)",
                            align=TRUE,
                            se = se1,
                            column.labels=c("Households","Non-Financial","Financial","Households","Non-Financial","Financial"),
                            column.separate = c(3,3,3,1,1,1),
                        ...
                        add.lines = list(bp,bp.star,ht,ht.star,st,st.star,ar1,ar1.star,ar2,ar2.star),
                        #add.lines = list(st1,ar1,ar2),
                           out="Research_Proposal/Progress/Tables/Final/Panel/Static_Models_SELEC_ext_75.html"))
...
#coeftest(OLS_plm[[1]], function(x) vcovHC(x,method= "arellano", type="HC3", cluster="group"))
  ...
  options(scipen=0, digits=3)
  haus_test.hh <- phtest(FE_plm[[1]], RE_plm[[1]])
  haus_test.fi <-phtest(FE_plm[[2]], RE_plm[[2]])
  haus_test.nfi <-phtest(FE_plm[[3]], RE_plm[[3]])
  ...
  stargazer(re.1,  fe.1,re.2 ,fe.2 , re.3,fe.3,sysgmm.1, diffgmm.2,diffgmm.3,title="Panel Data Models  (2014-2019)",
            ...
            #add.lines = list(st1,ar1,ar2),
            out="Research_Proposal/Progress/Tables/Final/Panel/Static_Models_SELEC_HCArell.tex")
  ...
  capture.output( stargazer(re.1,  fe.1,re.2 ,fe.2 , re.3,fe.3,sysgmm.1, diffgmm.2,diffgmm.3, title="Panel Data Models (2014-2019)",
                            ...
                            add.lines = list(ht,ht.star,st,st.star,ar1,ar1.star,ar2,ar2.star),
                            #add.lines = list(st1,ar1,ar2),
                            out="Research_Proposal/Progress/Tables/Final/Panel/Static_Models_SELEC_HCArell.html"))
