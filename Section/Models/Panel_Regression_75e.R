...
OLS_75e_model <- function(df) {
  plm(def_model_ALL, data = df, model = "pooling")
}

(OLS_75e_plm <- map(sets.75e,OLS_75e_model))
(OLS_75e_summary <- map(OLS_75e_plm,summary))
(OLS_75e_coef <- map(OLS_75e_summary ,coef))
...
(OLS_bk_plm <- map(sets.75e,OLS_bk_model))
(OLS_bk_summary <- map(OLS_bk_plm,summary))
(OLS_bk_coef <- map(OLS_bk_summary ,coef))

OLS_mk_75e_model <- function(df) {
  plm(def_model_MACR, data = df, model = "pooling")
}

(OLS_mk_75e_plm <- map(sets.75e,OLS_mk_75e_model))
(OLS_mk_75e_summary <- map(OLS_mk_75e_plm,summary))
(OLS_mk_75e_coef <- map(OLS_mk_75e_summary ,coef))

map(OLS_mk_75e_plm ,function(x) summary(x , vcov = vcovHC(x ,method= "arellano", type="sss", cluster="group"),diagnostics=T))
#Pooling-OLS with Robust covariance matrix estimators/clustered standard errors
...
(OLS2_plm <- map(sets.75e,OLS2_model))
...
(GLS_plm <- map(sets.75e,GLS_model))
...
(GLS2_plm <- map(sets.75e,GLS2_model))
...
FD_mk_75e_model <- function(df) {
  plm(def_model_MACR_diff , data=df,model = "fd")
}


(FD_mk_75e_plm <- map(sets.75e,FD_mk_75e_model))
(FD_mk_75e_summary <- map(FD_mk_75e_plm,summary))
(FD_mk_75e_coef <- map(FD_summary ,coef))
...
(FD2_plm <- map(sets.75e,FD2_model))
...
FE_75e_model <- function(df) {
  #plm(def_model_ALL, data = df, model = "within")
  plm(def_model_ALL, data = df, model = "within")
}

(FE_75e_plm <- map(sets.75e,FE_75e_model))
(FE_75e_summary <- map(FE_75e_plm,summary))
(FE_75e_coef <- map(FE_75e_summary ,coef))
...
FE_bk_75e_model <- function(df) {
  plm(def_model_BANK, data = df, model = "within")
}


(FE_bk_75e_plm <- map(sets.75e,FE_bk_75e_model))
(FE_bk_75e_summary <- map(FE_bk_75e_plm,summary))
(FE_bk_75e_coef <- map(FE_bk_75e_summary ,coef))
...
FE_mk_75e_model <- function(df) {
  plm(def_model_MACR, data = df, model = "within",effect="time")
}

(FE_mk_75e_plm <- map(sets.75e,FE_mk_75e_model))
(FE_mk_75e_summary <- map(FE_mk_75e_plm,summary))
(FE_mk_75e_coef <- map(FE_mk_75e_summary ,coef))
...
map(FE_mk_75e_plm, function(x) summary(x , vcov = vcovHC(x ,method= "white1", type="HC3", cluster="group")))

FE2_mk_75e_model <- function(df) {
  plm(def_model_MACR_Year, data = df, model = "within",effect="time")
}

(FE2_mk_75e_plm <- map(sets.75e,FE2_mk_75e_model))
(FE2_mk_75e_summary <- map(FE2_mk_75e_plm,summary))
(FE2_mk_75e_coef <- map(FE2_mk_75e_summary ,coef))


map(FE2_mk_75e_plm, function(x) summary(x , vcov = vcovHC(x ,method= "white1", type="HC3", cluster="group")))

FE2_bk_model <- function(df) {
  plm(def_model_BANK_Year, data = df, model = "within",effect="time")
}

(FE2_bk_plm <- map(sets.75e,FE2_bk_model))
...
(FE2_plm <- map(sets.75e,FE2_model))
...
(FE3_plm <- map(sets.75e,FE3_model))
...
(RE_75e_plm <- map(sets.75e,RE_75e_model))
(RE_75e_summary <- map(RE_75e_plm,summary))
(RE_75e_coef <- map(RE_75e_summary ,coef))

#One-way Random Effects with Robust covariance matrix estimators/clustered standard errors

map(RE_plm, function(x) summary(x , vcov = vcovHC(x ,method= "white1", type="sss", cluster="group")))

RE_mk_75e_model <- function(df) {
  #plm(def_model_ALL, data = df, model = "random", random.method = "swar")
...
(RE_mk_75e_plm <- map(sets.75e,RE_mk_75e_model))
(RE_mk_75e_summary <- map(RE_mk_75e_plm,summary))
(RE_mk_75e_coef <- map(RE_mk_75e_summary ,coef))

map(RE_mk_75e_plm, function(x) summary(x , vcov = vcovHC(x ,method= "white1", type="sss", cluster="group")))

RE2_mk_75e_model <- function(df) {
  #plm(def_model_ALL, data = df, model = "random", random.method = "swar")
...
(RE2_mk_75e_plm <- map(sets.75e,RE2_mk_75e_model))
(RE2_mk_75e_summary <- map(RE2_mk_75e_plm,summary))
...
(RE2_bk_75e_plm <- map(sets.75e,RE2_bk_75e_model))
(RE2_bk_75e_summary <- map(RE2_bk_75e_plm,summary))
(RE2_bk_75e_coef <- map(RE2_bk_75e_summary ,coef))
...
(RE_bk_75e_plm <- map(sets.75e,RE_bk_75e_model))
(RE_bk_75e_summary <- map(RE_bk_75e_plm,summary))
(RE_bk_75e_coef <- map(RE_bk_75e_summary ,coef))
...
(RE2_plm <- map(sets.75e,RE2_model))
...
(diffgmm_plm <- map(sets.75e,npl_model_diffgmm))
...
(diffgmm_mk_plm <- map(sets.75e,npl_model_mk_diffgmm))
...
(diffgmm2_plm <- map(sets.75e,npl_model_diffgmm2))
...
(diffgmm2_bk_plm <- map(sets.75e,npl_model_bk_diffgmm2))
...
(diffgmm2_mk_plm <- map(sets.75e,npl_model_mk_diffgmm2))
...
(diffgmm3_plm <- map(sets.75e,npl_model_diffgmm3))
...
(diffgmm4_plm <- map(sets.75e,npl_model_diffgmm4))
...
(sysgmm_plm <- map(sets.75e,npl_model_sysgmm))
...
(sysgmm_mk_plm <- map(sets.75e,npl_model_mk_sysgmm))
...
(sysgmm2_plm <- map(sets.75e,npl_model_sysgmm2))
...
(sysgmm3_plm <- map(sets.75e,npl_model_sysgmm3))
(sysgmm3_summary <- map(sysgmm3_plm,summary))
(sysgmm3_coef <- map(sysgmm3_summary ,coef))

#twosteps & twoways
npl_model_75e_sysgmm4 <- function(df) {
  pgmm(data_model, data = df, model="twosteps", effect= "twoways",  transformation = "ld")
}

(sysgmm4_75e_plm <- map(sets.75e,npl_model_75e_sysgmm4))
(sysgmm4_75e_summary <- map(sysgmm4_75e_plm,summary))
(sysgmm4_75e_coef <- map(sysgmm4_75e_summary ,coef))

npl_model_mk_75e_sysgmm4 <- function(df) {
  pgmm(def_model_ALL_gmmdd_mk_a , data = df, model="twosteps", effect= "twoways",  transformation = "ld")
}

(sysgmm4_mk_75e_plm <- map(sets.75e,npl_model_mk_75e_sysgmm4))
(sysgmm4_mk_75e_summary <- map(sysgmm4_mk_75e_plm,summary))
(sysgmm4_mk_75e_coef <- map(sysgmm4_mk_75e_summary ,coef))

npl_model_bk_75e_sysgmm4 <- function(df) {
  pgmm(def_model_ALL_bk_gmmdd_a , data = df, model="twosteps", effect= "twoways",  transformation = "ld")
}

(sysgmm4_bk_75e_plm <- map(sets.75e,npl_model_bk_75e_sysgmm4))
(sysgmm4_bk_75e_summary <- map(sysgmm4_bk_75e_plm,summary))
(sysgmm4_bk_75e_coef <- map(sysgmm4_bk_75e_summary ,coef))
...
(sysgmm4y_plm <- map(sets.75e,npl_model_sysgmm4y))
...
(pggls_plm <- map(sets.75e,npl_model_pggls))
...
(fegls_plm <- map(sets.75e,npl_model_fegls))
