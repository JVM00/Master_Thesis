...
View(R2rq_npl_hh.nfi)
View(R2rq_npl_fi)
View(R2rq_npl_nfi)

path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
save(QR_Y_mk_npl_hh.nfi,file=paste(path_data,'QR_Y_mk_npl_hh.nfi.RData', sep = ""))
save(QR_Y_mk_npl_hh,file=paste(path_data,'QR_Y_mk_npl_hh.RData', sep = ""))
save(QR_Y_mk_npl_nfi,file=paste(path_data,'QR_Y_mk_npl_nfi.RData', sep = ""))
save(QR_Y_mk_npl_fi,file=paste(path_data,'QR_Y_mk_npl_fi.RData', sep = ""))
...
t <- round(R2rq_npl_hh,3)
t1 <- round(R2rq_npl_fi,3)
t2 <- round(R2rq_npl_nfi,3)
 ...
 # tests <- list(m1.tn ,m1.pvalue,si.tn,si.pvalue,si1.tn,si1.pvalue,si2.tn,si2.pvalue,si3.tn,si3.pvalue,si4.tn,si4.pvalue)
  ...
  stargazer( rq_fit1, rq_fit2,rq_fit3,rq_fit4,rq_fit5,rq_fit6,rq_fit7,rq_fit8,rq_fit9,rq.se = "boot",
            ...
            add.lines = list(l,m1.tn ,m1.pvalue,si.tn,si.pvalue),
             digits=3, align=T, title= paste("Quantil Regression (2014-2019):",k,sep=" "),
             out=paste("Research_Proposal/Progress/Tables/Final/quantil/Quantil_Regression_y_mk_",d,".html", sep = ""))
  ...
  capture.output(stargazer( rq_fit1, rq_fit2,rq_fit3,rq_fit4,rq_fit5,rq_fit6,rq_fit7,rq_fit8,rq_fit9,rq.se = "boot",
                            ...
                            add.lines = list(l,m1.tn ,m1.pvalue,si.tn,si.pvalue),
                            digits=3, align=T, title= paste("Quantil Regression (2014-2019):",k,sep=" ")
                            ,out=paste("Research_Proposal/Progress/Tables/Final/quantil/Quantil_Regression_y_mk_",d,".tex", sep = "")))
} 
...
for (i in names(anova)){
 ...
 print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_Y_mk_",d,".html", sep = ""))
print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_Y_mk_",d,".tex", sep = ""))
...
QR_npl_hh <- QR_Y_mk_npl_hh %>%filter(!grepl('Year', var))%>%  mutate(Exposure = "Households")
QR_npl_nfi <- QR_Y_mk_npl_nfi %>%filter(!grepl('Year', var))%>% mutate(Exposure = "Non-Financial Corp.")
QR_npl_fi <- QR_Y_mk_npl_fi %>%filter(!grepl('Year', var))%>% mutate(Exposure = "Financial Corp.")
#View(QR_Y_mk_npl_hh)
#View(QR_Y_mk_npl_hh)
...
for (i in names(sets)){
  ...
  pfe.name<- names(npl_hh[all.vars(de_model_ALL_rqpd_Year_mk)])
  p <- npl_hh %>% group_by(Year) %>% slice(1) %>% count() %>% ungroup() %>% summarise(n_1=sum(n))
 
  a <- length(pfe.name) - 1 + (p - 2)
  b <- length(seq(.1,.9,.1))
  c <- a*b
  
  pfe2.table <- data.frame(pfe2.summ1$coefficients[1:c[1,1],])
  
 # nrow(pfe2.summ1$coefficients)
 ...
 # class( pfe2.table2$pfe.types)
  ...
  assign(paste("QRFE1_Y_mk_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh)
  ...
  pfe2.name <-pfe2.table2$pfe.row.names_mod[1:a[1,1]]
 ...
 # x <- xtable(pfe3.table2, caption = paste("A Fixed Effects Quantile Regression (2014-2019)[$\lambda = 1$]:",k,sep=" "))
  ...
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_Y_mk_",d,lambda,".html", sep = ""))
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_y_mk_",d,lambda,".tex", sep = ""))
...
path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QRFE1_npl_hh.nfi,file=paste(path_data,'QRFE1_npl_hh.nfi.RData', sep = ""))
save(QRFE1_Y_mk_npl_fi,file=paste(path_data,'QRFE1_Y_mk_npl_fi.RData', sep = ""))

save(QRFE1_Y_mk_npl_hh,file=paste(path_data,'QRFE1_Y_mk_npl_hh.RData', sep = ""))
save(QRFE1_Y_mk_npl_nfi,file=paste(path_data,'QRFE1_Y_mk_npl_nfi.RData', sep = ""))



#filter(!grepl('Year', var))%>%

# ggplot(QRFE1_Y_npl_hh.nfi, aes(x=pfe.types)) + 
#   xlab("Quantile(%)") +
#   ylab(" ")+
...
QR_npl <- as.data.frame(bind_rows(QR_npl_hh,QR_npl_nfi,QR_npl_fi))
View(QRFE1_Y_mk_npl_fi)
pfe.row.names_mod

QRFE1_hh <- QRFE1_Y_mk_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")

ggplot(QRFE1_hh,aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 1 ): households")

ggplot(QRFE_Y_mk_npl_nfi, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
...
for (i in names(sets)){
  ...
  assign(paste("QRFE5_Y_mk_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh)
...
path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QRFE1_npl_hh.nfi,file=paste(path_data,'QRFE1_npl_hh.nfi.RData', sep = ""))
save(QRFE5_Y_mk_npl_fi,file=paste(path_data,'QRFE5_Y_mk_npl_fi.RData', sep = ""))

save(QRFE5_Y_mk_npl_hh,file=paste(path_data,'QRFE5_Y_mk_npl_hh.RData', sep = ""))
save(QRFE5_Y_mk_npl_nfi,file=paste(path_data,'QRFE5_Y_mk_npl_nfi.RData', sep = ""))

QRFE5_hh <- QRFE5_Y_mk_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")

ggplot(QRFE5_hh,aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 5 ): households")
...
for (i in names(sets)){
  ...
  assign(paste("QRFE2.5_Y_mk_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh)
...
path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QRFE1_npl_hh.nfi,file=paste(path_data,'QRFE1_npl_hh.nfi.RData', sep = ""))
save(QRFE2.5_Y_mk_npl_fi,file=paste(path_data,'QRFE2.5_Y_mk_npl_fi.RData', sep = ""))

save(QRFE2.5_Y_mk_npl_hh,file=paste(path_data,'QRFE2.5_Y_mk_npl_hh.RData', sep = ""))
save(QRFE2.5_Y_mk_npl_nfi,file=paste(path_data,'QRFE2.5_Y_mk_npl_nfi.RData', sep = ""))


QRFE2.5_hh <- QRFE2.5_Y_mk_npl_hh %>%filter(!grepl('Year', pfe.row.names_mod))%>% mutate(Exposure = "Financial Corp.")

ggplot(QRFE2.5_hh,aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 2.5 ): households")
...
#   
#   
#   pfe.name<- names(npl_hh[all.vars(de_model_ALL_rqpd_Year)])
#   
#   a <- length(pfe.name) - 1
#   b <- length(seq(.1,.9,.1))
#   c <- a*b
...
#   # class( pfe2.table2$pfe.types)
#   
#   assign(paste("QRFE5_Y_",d, sep= ""),pfe2.table2)
#   # View(QRFE_npl_hh)
#   
#   for(j in 1:nrow(pfe2.table2)) {
#     pfe2.table2[j, 1] <- paste0(pfe2.table2[j, 1], starPrinter(cf_10.pVals[j]))
...
#   
#   print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
#                  file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE5_Y_",d,lambda,".html", sep = ""))
#   print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
#                  file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE5_Y_",d,lambda,".tex", sep = ""))
# }
# 
# path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
# save(QRFE5_Y_npl_hh.nfi,file=paste(path_data,'QRFE5_npl_hh.nfi.RData', sep = ""))
# save(QRFE5_Y_npl_fi,file=paste(path_data,'QRFE5_npl_fi.RData', sep = ""))
# 
# save(QRFE5_Y_npl_hh,file=paste(path_data,'QRFE5_npl_hh.RData', sep = ""))
# save(QRFE5_Y_npl_nfi,file=paste(path_data,'QRFE5_npl_nfi.RData', sep = ""))
# 
# #View(QRFE_npl_hh)
# ggplot(QRFE5_Y_npl_hh.nfi, aes(x=pfe.types)) + 
#   xlab("Quantile(%)") +
#   ylab(" ")+
#   facet_wrap(~ pfe.row.names_mod,scales = "free")+
#   geom_line(aes(y = Value), color = "black",size=.6) + 
...
# 
# ggplot(QRFE5_Y_npl_fi, aes(x=pfe.types)) + 
#   xlab("Quantile(%)") +
#   ylab(" ")+
#   facet_wrap(~ pfe.row.names_mod,scales = "free")+
#   geom_line(aes(y = Value), color = "black",size=.6) + 
...
# 
# # ggplot(QRFE_Y_npl_nfi, aes(x=pfe.types)) + 
# #   xlab("Quantile(%)") +
# #   ylab(" ")+
# #   facet_wrap(~ pfe.row.names_mod,scales = "free")+
# #   geom_line(aes(y = Value), color = "black",size=.6) + 
...
#   
#   print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
#                  file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE2.5_Y_",d,lambda,".html", sep = ""))
#   print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
#                  file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE2.5_Y_",d,lambda,".tex", sep = ""))
# }
# 
# path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
# #save(QRFE2.5_npl_hh.nfi,file=paste(path_data,'QRFE2.5_npl_hh.nfi.RData', sep = ""))
# save(QRFE2.5_Y_npl_fi,file=paste(path_data,'QRFE2.5_Y_npl_fi.RData', sep = ""))
# 
# save(QRFE2.5_Y_npl_hh,file=paste(path_data,'QRFE2.5_Y_npl_hh.RData', sep = ""))
# save(QRFE2.5_Y_npl_nfi,file=paste(path_data,'QRFE2.5_Y_npl_nfi.RData', sep = ""))
# 
# View(QRFE_npl_hh)
# ggplot(QRFE2.5_Y_npl_hh.nfi, aes(x=pfe.types)) + 
#   xlab("Quantile(%)") +
#   ylab(" ")+
#   facet_wrap(~ pfe.row.names_mod,scales = "free")+
#   geom_line(aes(y = Value), color = "black",size=.6) + 
...
# 
# ggplot(QRFE2.5_Y_npl_fi, aes(x=pfe.types)) + 
#   xlab("Quantile(%)") +
#   ylab(" ")+
#   facet_wrap(~ pfe.row.names_mod,scales = "free")+
#   geom_line(aes(y = Value), color = "black",size=.6) + 
...
# cre.fit4 <- rqpd(de_model_ALL_rqpd_cre, panel(taus= 0.75, method="cre",cre="crem"), data=npl_hh)
...
#   return("")
# }
# # sets_pfe <- list(pfe.summ1=pfe.summ1,pfe.summ2=pfe.summ2,pfe.summ3=pfe.summ3,
# #                  pfe.summ4=pfe.summ4,pfe.summ5=pfe.summ5,pfe.summ6=pfe.summ6,
# #                  pfe.summ7=pfe.summ7,pfe.summ8=pfe.summ9,pfe.summ9=pfe.summ9)
# 
# sets_cre <- list(cre.summ1=cre.summ1, cre.summ2=cre.summ2, cre.summ3=cre.summ3,cre.summ4=cre.summ4,cre.summ5=cre.summ5)
# 
# 
# myEstVector <- c()
# for (i in names(sets_cre)){
#   
#   a <- i
#   b <- names( sets_cre)
#   c <- match(a,b)
