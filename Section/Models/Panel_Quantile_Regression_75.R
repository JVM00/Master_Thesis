...
#rq_fit <- rq(def_model_ALL,  tau = seq(0.1,0.9,0.1),data = npl_hh.75)
...
# rq_fit4 <- rq(dat,  tau = .75 ,data = datset)
# rq_fit5 <- rq(dat,  tau = .9,data = datset)
# 
# #rq_summ1 <- summary(rq_fit1 ,se="boot", bsmethod="cluster",cluster= npl_hh.75$name )
...
# rq_fit11 <- rq(dat,  tau = .75,data = datset)
...
#   
#   
#   rq_coef4 <-  data.frame(summary(rq_fit4 ,se="rank")$coefficients) %>% mutate(type= .75,var=row.names(summary(rq_fit4,se="rank")$coefficients),pval=rq_pval4,se= rq_se4,pval_type=ifelse(pval <0.01,"***",ifelse(pval <0.05,"**", ifelse(pval <0.1,"*"," "))))
#   
#   
...
View(R2rq_npl_hh.75.nfi)
View(R2rq_npl_fi.75)
View(R2rq_npl_nfi.75)

path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QR_npl_hh.75.nfi,file=paste(path_data,'QR_npl_hh.75.nfi.RData', sep = ""))
save(QR_npl_hh.75,file=paste(path_data,'QR_npl_hh.75.RData', sep = ""))
save(QR_npl_nfi.75,file=paste(path_data,'QR_npl_nfi.75.RData', sep = ""))
save(QR_npl_fi.75,file=paste(path_data,'QR_npl_fi.75.RData', sep = ""))
...
#t <- round(R2rq_npl_hh.75.nfi,3)
t <- round(R2rq_npl_hh.75,3)
t1 <- round(R2rq_npl_fi.75,3)
t2 <- round(R2rq_npl_nfi.75,3)
...
#anova <- list(ANOVA_npl_hh.75=ANOVA_npl_hh.75,ANOVA_npl_fi.75=ANOVA_npl_fi.75,ANOVA_npl_nfi.75=ANOVA_npl_nfi.75)

#anova <- list(ANOVA_npl_hh.75.nfi=ANOVA_npl_hh.75.nfi,ANOVA_npl_fi.75=ANOVA_npl_fi.75)
anova <- list(ANOVA_npl_hh.75=ANOVA_npl_hh.75,ANOVA_npl_nfi.75=ANOVA_npl_nfi.75,ANOVA_npl_fi.75=ANOVA_npl_fi.75)
class(anova)
anova[1]
#anova <- list(ANOVA_npl_fi.75=ANOVA_npl_fi.75)

#anova <- list(ANOVA_npl_hh.75.nfi=ANOVA_npl_hh.75.nfi)
...
for (i in names(anova)){
 ...
 print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_",d,".html", sep = ""))
print.xtable(  df1, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/Slope Test_",d,".tex", sep = ""))
...
# 
# anova_names <- c("Df","Resi Df","F-stat","P-val")
# names(ANOVA_npl_hh.75.coef) <- anova_names
# 
# ANOVA_npl_hh.75.coef[3] <- round(ANOVA_npl_hh.75.coef[3],4)
# ANOVA_npl_hh.75.coef[4] <- round(ANOVA_npl_hh.75.coef[4],5)
# 
...
QR_npl_hh.75 <- QR_npl_hh.75 %>% mutate(Exposure = "Households")
QR_npl_nfi.75 <- QR_npl_nfi.75 %>% mutate(Exposure = "Non-Financial Corp.")
QR_npl_fi.75 <- QR_npl_fi.75 %>% mutate(Exposure = "Financial Corp.")

QR_npl <- as.data.frame(bind_rows(QR_npl_hh.75,QR_npl_nfi.75,QR_npl_fi.75))
View(QR_npl)
...
ggplot(data=QR_npl_hh.75,aes(x=type,y = coefficients)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ var,scales = "free")+
  geom_line(color = "black", size=.6) +
  geom_line(data=QR_npl_nfi.75,color = "grey",size=.6) + 
  geom_line(data=QR_npl_fi.75, color = "blue",size=.6) + 
  #guides(color=guide_legend(title="My Legend Title")) 
  geom_point(data=QR_npl_hh.75,aes(color=pval_type),shape=16,size=1)+
  geom_point(data=QR_npl_nfi.75,aes(color=pval_type),shape=16,size=1)+
  geom_point(data=QR_npl_fi.75,aes(color=pval_type),shape=16,size=1)+
...
ggplot(QR_npl_hh.75.nfi, aes(x=type)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ var,scales = "free")+
...
ggplot(QR_npl_hh.75, aes(x=type)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ var,scales = "free")+
  geom_line(aes(y = coefficients), color = "black",size=.6) + 
...
ggplot(QR_npl_fi.75, aes(x=type)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ var,scales = "free")+
  geom_line(aes(y = coefficients), color = "black",size=.6) + 
...
ggplot(QR_npl_nfi.75, aes(x=type)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ var,scales = "free")+
  geom_line(aes(y = coefficients), color = "black",size=.6) + 
...
# quantile(npl_hh.75$NPL_ratio,seq(.1,.9,.1))
# summary(npl_hh.75$NPL_ratio)
# stat.desc(npl_hh.75$NPL_ratio)
#qqnorm(npl_hh.75$NPL_ratio, pch = 1, frame = FALSE)
...
for (i in names(sets.75)){
  ...
  pfe2.fit1 <- rqpd(dat, panel(lambda = lambda,taus= seq(.1,.9,.1),tauw=rep(1/9,9),method="pfe"), data= datset, control=sfn.control(tmpmax=20*nrow(npl_hh.75)) )
  
  pfe2.summ1 <- summary.rqpd(pfe2.fit1, se = "boot",covariance = TRUE)
  
  
  pfe.name<- names(npl_hh.75[all.vars(de_model_ALL_rqpd)])
  ...
  # QRFE_npl_hh.751 <-  QRFE_npl_hh.75 %>% mutate(pval_type=ifelse(Pr...t.. <0.01,"<0.01",ifelse( Pr...t.. <0.05,"<0.05", 
  #                                                                                        ifelse(Pr...t.. <0.1,"<0.1",">=0.1"))))
  # View( QRFE_npl_hh.751)
  # View( QRFE_npl_hh.75)
  
  #QRFE_npl_hh.751$var <- as.numeric(gsub("(\\[)(\\d*)(\\.)(\\d*)(\\])","", QRFE_npl_hh.751$var))
  #QRFE_npl_hh.751$pfe.types <- as.numeric(gsub("(\\[)|(\\])","", QRFE_npl_hh.751$pfe.types))
 ...
 # class( pfe2.table2$pfe.types)
  ...
  assign(paste("QRFE1_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh.75)
  ...
  #pfe2.name <-names(npl_hh.75[all.vars(de_model_ALL_nam)])
 ...
 # x <- xtable(pfe3.table2, caption = paste("A Fixed Effects Quantile Regression (2014-2019)[$\lambda = 1$]:",k,sep=" "))
  ...
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_",d,lambda,".html", sep = ""))
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE_",d,lambda,".tex", sep = ""))

  
  }


View(QRFE1_npl_hh.75.nfi)

path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
save(QRFE1_npl_hh.75.nfi,file=paste(path_data,'QRFE1_npl_hh.75.nfi.RData', sep = ""))
save(QRFE1_npl_fi.75,file=paste(path_data,'QRFE1_npl_fi.75.RData', sep = ""))

save(QRFE1_npl_hh.75,file=paste(path_data,'QRFE1_npl_hh.75.RData', sep = ""))
save(QRFE1_npl_nfi.75,file=paste(path_data,'QRFE1_npl_nfi.75.RData', sep = ""))

ggplot(QRFE1_npl_hh.75.nfi, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
 ...
 # geom_line(aes(y = upper.bd), color="lightblue3") +
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 1 ): Households & Non-Finanial Corporations")

ggplot(QRFE1_npl_fi.75, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 1 ): Financial Corporations")

ggplot(QRFE_npl_nfi.75, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
 # ggtitle(expression("Fixed Effects Quantile Regression" ~ lambda ))
   ggtitle(expression("Fixed Effects Quantile Regression ("~lambda~"= 1 ): Non-Financial Corporations"))
...
for (i in names(sets.75)){
 ...
 # j <- c("Households & Non-Financial Corporations","Financial Corporations")
  ...
  assign(paste("QRFE5_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh.75)
  ...
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE5_",d,lambda,".html", sep = ""))
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE5_",d,lambda,".tex", sep = ""))
}

path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
save(QRFE5_npl_hh.75.nfi,file=paste(path_data,'QRFE5_npl_hh.75.nfi.RData', sep = ""))
save(QRFE5_npl_fi.75,file=paste(path_data,'QRFE5_npl_fi.75.RData', sep = ""))

save(QRFE5_npl_hh.75,file=paste(path_data,'QRFE5_npl_hh.75.RData', sep = ""))
save(QRFE5_npl_nfi.75,file=paste(path_data,'QRFE5_npl_nfi.75.RData', sep = ""))

#View(QRFE_npl_hh.75)
ggplot(QRFE5_npl_hh.75.nfi, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 5 ): Households & Non-Finanial Corporations")

ggplot(QRFE5_npl_fi.75, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 5 ): Finanial Corporations")
...
ggplot(QRFE_npl_nfi.75, aes(x=pfe.types)) + 
  ...
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  # ggtitle(expression("Fixed Effects Quantile Regression" ~ lambda ))
  ggtitle(expression("Fixed Effects Quantile Regression ("~lambda~"= 5 ): Non-Financial Corporations"))
...
for (i in names(sets.75)){
 ...
 # j <- c("Households & Non-Financial Corporations","Financial Corporations")
  ...
  assign(paste("QRFE2.5_",d, sep= ""),pfe2.table2)
  # View(QRFE_npl_hh.75)
  ...
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="HTML",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE2.5_",d,lambda,".html", sep = ""))
  print.xtable(  x, caption.placement = 'top' ,align= "c",booktabs = T,size = "small", include.rownames= TRUE,type="latex",
                 file= paste("Research_Proposal/Progress/Tables/Final/quantil/QRFE2.5_",d,lambda,".tex", sep = ""))
}

path_data <- 'C:/Users/varga/Desktop/Thesis/R_data/Data/'
#save(QRFE2.5_npl_hh.75.nfi,file=paste(path_data,'QRFE2.5_npl_hh.75.nfi.RData', sep = ""))
save(QRFE2.5_npl_fi.75,file=paste(path_data,'QRFE2.5_npl_fi.75.RData', sep = ""))

save(QRFE2.5_npl_hh.75,file=paste(path_data,'QRFE2.5_npl_hh.75.RData', sep = ""))
save(QRFE2.5_npl_nfi.75,file=paste(path_data,'QRFE2.5_npl_nfi.75.RData', sep = ""))

View(QRFE_npl_hh.75)
ggplot(QRFE2.5_npl_hh.75.nfi, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
...
ggplot(QRFE2.5_npl_fi.75, aes(x=pfe.types)) + 
  xlab("Quantile(%)") +
  ylab(" ")+
  facet_wrap(~ pfe.row.names_mod,scales = "free")+
  geom_line(aes(y = Value), color = "black",size=.6) + 
  ...
  labs(caption = "Source: EBA Transparency Exercises, authors' calculations")+
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  ggtitle("Fixed Effects Quantile Regression ("~lambda~"= 2.5 ): Finanial Corporations")
...
ggplot(QRFE_npl_nfi.75, aes(x=pfe.types)) + 
  ...
  theme(legend.position="bottom",legend.direction = "horizontal",legend.spacing.x = unit(.5, 'cm'))+
  # ggtitle(expression("Fixed Effects Quantile Regression" ~ lambda ))
  ggtitle(expression("Fixed Effects Quantile Regression ("~lambda~"= 2.5): Non-Financial Corporations"))
...
# 
# 
# pfe2.fit1 <- rqpd(de_model_ALL_rqpd, panel(lambda = 1,taus= seq(.1,.9,.1),tauw=rep(1/9,9),method="pfe"), data=npl_hh.75, control=sfn.control(tmpmax=20*nrow(npl_hh.75)) )
# 
# pfe2.summ1 <- summary.rqpd(pfe2.fit1, se = "boot",covariance = TRUE)
# 
# # qqplot(npl_hh.75$NPL_ratio)
# # library("car")
# # qqPlot(npl_hh.75$NPL_ratio)
# 
# 
...
# library(sdcMicro)
# 
# # npl_hh.751 <- npl_hh.75
# # npl_hh.75_1 <- addNoise(data.frame(npl_hh.75), variables =  c("NPL_ratio","CET1_ratio_tr","LEVER_tr_ratio_imput", "RORAC","BankSize_tran_imput", "Cost_to_Income" ,      "NIM_tr_imput"  ,       "Stgth_legal_index" ,   "Reco_rate_dol"    ,    "infl_perc"   ,        
# #                                                         "curr_acc_bal"   , "GDP_growth_perc"   ,"unempl_rate_imf"  , "gross_public_debt"   ,          
# #   
# #                                                                                                                                                         "CBDI_index"   ) , noise = 20,method = "additive") 
# # npl_hh.752 <-   data.frame( npl_hh.75_1$xm    )    
# 
# #names(npl_hh.75[all.vars(def_model_ALL)])
# npl_hh.75$NPL_ratio
#  options(dplyr.width = Inf)
# 
# cre.fit1 <- rqpd(de_model_ALL_rqpd_cre, panel(taus= seq(.25,.75,.25), method="cre",cre="crem"), 
#                  data=npl_hh.75,control=sfn.control(tmpmax=2.5*nrow(npl_hh.75),small=0.000000000000001,cachsz=100))
# cre.fit2 <- rqpd(de_model_ALL_rqpd_cre, panel( method="cre",cre="m"), data=p1)
# cre.fit3 <- rqpd(de_model_ALL_rqpd_cre, panel(taus= .5, method="cre",cre="crem"), data=npl_hh.75)
# cre.fit4 <- rqpd(de_model_ALL_rqpd_cre, panel(taus= 0.75, method="cre",cre="crem"), data=npl_hh.75)
# cre.fit5 <- rqpd(de_model_ALL_rqpd_cre, panel(taus= 0.9, method="cre",cre="crem"), data=npl_hh.75)
# 
...
#   return("")
# }
# # sets.75_pfe <- list(pfe.summ1=pfe.summ1,pfe.summ2=pfe.summ2,pfe.summ3=pfe.summ3,
# #                  pfe.summ4=pfe.summ4,pfe.summ5=pfe.summ5,pfe.summ6=pfe.summ6,
# #                  pfe.summ7=pfe.summ7,pfe.summ8=pfe.summ9,pfe.summ9=pfe.summ9)
# 
# sets.75_cre <- list(cre.summ1=cre.summ1, cre.summ2=cre.summ2, cre.summ3=cre.summ3,cre.summ4=cre.summ4,cre.summ5=cre.summ5)
# 
# 
# myEstVector <- c()
# for (i in names(sets.75_cre)){
#   
#   a <- i
#   b <- names( sets.75_cre)
#   c <- match(a,b)
#   d <-  b[c]
#   e <- names(sets.75_cre[c])
#   f <-  sets.75_cre[[c]]
#   
#   # coefficients and t-values in matrix
...
# colnames(myNewTable) <- c(0.10,0.25,0.5,0.75,0.9)
# View(myNewTable)
# 
# 
# x <- xtable(pfe3.table2, caption = "A Fixed Effects Quantile Regression")
