...
#--------------------------------POOLING---------------------------------------------
#Pooling-OLS
#t <- update(def_model_ALL,.~.-Reco_rate_dol + TimTOres_insol)

#def_model_ALL <- update(def_model_ALL,.-SSM - CBDI_index)
def_model_BANK1 <-  update(def_model_BANK, .~.- SSM)
...
FE_bk_model <- function(df) {
  plm(def_model_BANK1, data = df, model = "within")
}
...
map(RE_plm, function(x) summary(x , vcov = vcovHC(x ,method= "arellano", type="sss", cluster="group")))
