...
/**************************BEGIN--RESOLVING ISSUE WITH MISSING DATA AND EXTRAPOLNG DATA via R FOR ALL VARIABLES USING LEVERAGE EXPOSURES**/

/*
bankspec.nim_tr 
bankspec.banksize_tran
bankspec.LEVER_tr_ratio_calc
bankspec.lever_tr_ratio

'2020903'n AS lever_tr_calc_deno,
'2020903'n AS  nim_tr_deno

bankspec.FORBEAR_RATIO
Calculation: Perf_Status={'0'} Sum(2020705)/ Perf_Status={'0'}Sum([2020605])
...
Libname Macrdata '/mnt/sasshare/DWH/Adhocs/Javier_V/ECB/sas_data/MacroData';
Libname BankSpec '/mnt/sasshare/DWH/Adhocs/Javier_V/ECB/sas_data/BankSpecData' ; 
Libname update '/mnt/sasshare/DWH/Adhocs/Javier_V/ECB/update';



DATA base;*need two indices for extrapoletion name and exposure;
SET bankspec.NPL_RATIOS(KEEP=Period_nm LEI_Code name nsa nsa_label exposure) ;
RUN; 
PROC FREQ DATA= base; TABLE Period_nm; RUN;

PROC SORT DATA= base OUT= base_nb NODUPKEY;BY Period_nm LEI_Code; RUN;

PROC FREQ DATA= base_nb; TABLE Period_nm; RUN;

DATA nim_tr;
SET  bankspec.nim_tr (KEEP= Period_nm LEI_Code name nsa nsa_label '2020301'n '2020304'n '2020903'n NIM_tr);

RUN; 

DATA banksize_tran;
SET  bankspec.banksize_tran(KEEP= Period_nm LEI_Code name nsa nsa_label BankSize_tran) ;
RUN; 

DATA LEVER_tr_ratio_calc;
SET  bankspec.LEVER_tr_ratio_calc(KEEP= Period_nm LEI_Code name nsa nsa_label  '2020901'n '2020903'n LEVER_tr_ratio_calc);
RUN; 

DATA lever_tr_ratio;
SET  bankspec.lever_tr_ratio(KEEP= Period_nm LEI_Code name nsa nsa_label LEVER_tr_ratio );
RUN; 
...
DATA FORBEAR_RATIO;
SET BANKSPEC.FORBEAR_RATIO (KEEP= Period_nm LEI_Code name nsa exposure '2020605'n '2020705'n Forbear_ratio);
*IF '2020605'n <= 0 THEN '2020605'n = '.';
*IF '2020705'n  <= 0 THEN '2020705'n  = '.'; 

*IF Forbear_ratio  <= 0 THEN Forbear_ratio  = '.';
RUN; 

PROC SQL;
	CREATE TABLE update.base_nb1  AS
	SELECT 
	a.*,
	b.'2020903'n AS  nim_tr_deno,
	(b.'2020301'n - b.'2020304'n) AS  nim_tr_nume,
	b.NIM_tr,
	c.banksize_tran,
	d.LEVER_tr_ratio_calc,
	d.'2020903'n AS lever_tr_calc_deno,
    d.'2020901'n AS lever_tr_calc_nume,
	e.lever_tr_ratio
	FROM  base_nb AS a
		LEFT JOIN nim_tr AS b
		ON a.Period_nm=b.Period_nm AND a.LEI_Code= b.LEI_Code
			LEFT JOIN banksize_tran AS c
		ON a.Period_nm=c.Period_nm AND a.LEI_Code= c.LEI_Code
				LEFT JOIN LEVER_tr_ratio_calc AS d
		ON a.Period_nm=d.Period_nm AND a.LEI_Code= d.LEI_Code
				LEFT JOIN lever_tr_ratio AS e
		ON a.Period_nm=e.Period_nm AND a.LEI_Code= e.LEI_Code;
QUIT;


data update.base_nb1(drop = i);
   set update.base_nb1;
   array vars{*}  _NUMERIC_;
;
...
PROC SQL;
	CREATE TABLE update.base1  AS
	SELECT 
	a.*,
	b.'2020605'n AS forbear_deno,
	b.'2020705'n AS forbear_nume, 
	b.Forbear_ratio
	FROM  base AS a
		LEFT JOIN FORBEAR_RATIO AS b
		ON a.Period_nm=b.Period_nm AND a.LEI_Code= b.LEI_Code AND a.exposure=b.exposure;
QUIT;



data update.base1(drop = i);
   set update.base1;
   array vars{*}  _NUMERIC_;
;
